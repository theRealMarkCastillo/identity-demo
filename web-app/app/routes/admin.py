...
def _collect_admin_data() -> dict:
    """Run the same queries the SSR template uses; return JSON-friendly dicts.

    Shared between the SSR pass and the `/api/admin-data` JSON endpoint so
    that what the dashboard renders server-side is the same shape the JS
    polls and re-renders client-side.
    """
    with get_conn() as conn, conn.cursor() as cur:
        cur.execute("""
            SELECT r.role, r.description,
        ...
        tokens = [
            {
                "jti_short": str(r[0])[:12] + "...",
                "sub": r[1], "act_sub": r[2], "client_id": r[3],
                "scope": r[4], "exp": r[5].isoformat() if r[5] else None,
...
def admin_data_api(request: Request):
    """JSON snapshot of every panel on the admin dashboard.

    The SSR pass uses the same `_collect_admin_data()` so the initial paint
    matches what the client-side polls see -- no shape drift between
    server-render and live-update.
    """
    if not _require_login(request):
        raise HTTPException(status_code=401, detail="not logged in")
    return JSONResponse(_collect_admin_data())