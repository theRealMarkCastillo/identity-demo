...
## Verification
59 unit + integration tests covering all three principal types (human direct, delegated agent, headless agent), multi-hop delegation chains, Cedar policy logic, and column-level masking paths. Run `make test` after `make up`. Eight one-click demo buttons in the dashboard, including a 3-hop delegation chain and a side-by-side raw-vs-masked diff per PII cell.
