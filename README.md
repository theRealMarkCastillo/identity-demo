...
## Verification
55+ unit + integration tests covering all three principal types (human direct, delegated agent, headless agent) plus Cedar policy logic and column-level masking paths. Run `make test` after `make up`. Eight one-click demo buttons in the dashboard, including side-by-side raw-vs-masked diff per PII cell.
