...
- **Agent ownership is recorded, not governed.** `platform.agents.owner_user_id` (required, set via `cli-admin agent add --owner`) answers "who's accountable for this agent existing and having these scopes" — a real gap otherwise, since a headless agent's own JWTs never carry a human identifier at all (`sub=agent_id`, no `act`), so without this column there'd be *no* traceable human for that entire principal type, not even at provisioning time. But it's deliberately just a pointer, not a managed relationship: no expiry, no notification when access is about to lapse, and critically no transfer-on-departure — if the owner account were deleted or left the org, nothing in this demo reassigns or flags orphaned agents. That's the identity-*governance* layer (agent lifecycle, sponsorship, succession), which this demo doesn't attempt to build. Microsoft Entra Agent ID's "Sponsor" concept is the reference example of what a real implementation looks like: a managed relationship with expiry notifications and automatic sponsorship transfer to the sponsor's manager on departure (see §14). Recording *who* is in scope here; managing *what happens over that person's lifecycle* is not.
...
2. **Microsoft Entra Agent ID** (2026) is a separate, newer, agent-specific product — and it isn't primarily a token-issuance mechanism at all. It's an identity *governance* platform: agents (including ones built on non-Microsoft platforms, integrated via the Entra Auth SDK sidecar or workload identity federation) get identity accounts inside your Entra tenant, with conditional access, risk detection, and lifecycle management layered on top. Ownership specifically is a **Sponsor** — "human users accountable for making decisions about \[an agent identity's\] lifecycle and access" — a *managed relationship*, not just a pointer: sponsors get notified before access expires, can request or extend access on the agent's behalf, and sponsorship automatically transfers to the sponsor's manager if they leave the org. This demo's `platform.agents.owner_user_id` (§13) records the same "who's accountable" fact but stops there — no expiry tracking, no transfer, no notifications. It requires being an Entra tenant customer with specific licensing (Entra ID P1/P2, or Microsoft 365 E5/E7 plus a Microsoft Agent 365 license). Adopting it replaces more than this demo's control plane — it replaces the *governance* layer this demo deliberately doesn't attempt to build (no policy versioning, no lifecycle management, no conditional access — see §13).
